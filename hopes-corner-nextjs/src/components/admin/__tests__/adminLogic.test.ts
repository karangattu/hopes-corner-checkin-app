...
describe('admin component logic', () => {
    ...
    describe('BatchUploadSection logic', () => {
        describe('CSV parsing', () => {
            it('splits CSV by newlines', () => {
                const csv = 'name,age\nJohn,30\nJane,25';
            ...
            it('parses header row', () => {
                ...
                expect(headers).toEqual(['firstName', 'lastName', 'age']);
            });
            ...
            it('handles quoted values', () => {
                ...
                expect(unquoted).toBe('Doe, John');
            });
        });

        describe('validation', () => {
            it('validates required fields', () => {
                const row = { firstName: 'John', lastName: '' };
                const hasRequiredFields = row.firstName && row.lastName;
                expect(hasRequiredFields).toBeFalsy();
            });

            it('counts validation errors', () => {
                const rows = [
                    { firstName: 'John', lastName: 'Doe' },
                    { firstName: '', lastName: 'Smith' },
                    { firstName: 'Jane', lastName: '' },
                ];
                const errors = rows.filter(r => !r.firstName || !r.lastName);
