...
const Dashboard = () => {
  ...
  const monthMetrics = useMemo(() => {
    ...
    const monthBicycles = (bicycleRecords||[]).filter(r => inMonth(r.date));
  ...
  }, [mealRecords, showerRecords, laundryRecords, haircutRecords, holidayRecords, bicycleRecords]);
  ...
  const yearMetrics = useMemo(() => {
    ...
    const yearBicycles = (bicycleRecords||[]).filter(r => inYear(r.date));
    const countsAsLaundryLoad = (rec) => rec.laundryType === 'onsite' ? (rec.status === 'DONE' || rec.status === 'PICKED_UP') : true;
    return {
  ...
  <Animated.div style={overviewGridAnim} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <Animated.div style={monthGridAnim} className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <div className="bg-white border rounded-lg p-4">
              <div className="flex items-center gap-2 text-gray-600 mb-1">Haircuts</div>
              <div className="text-2xl font-bold">{monthMetrics.haircuts || 0}</div>
            </div>
            <div className="bg-white border rounded-lg p-4">
              <div className="flex items-center gap-2 text-gray-600 mb-1">Holiday</div>
              <div className="text-2xl font-bold">{monthMetrics.holidays || 0}</div>
            </div>
            <div className="bg-white border rounded-lg p-4">
              <div className="flex items-center gap-2 text-gray-600 mb-1">Bicycle</div>
