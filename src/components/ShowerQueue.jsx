      ...
      {queue.length === 0 ? (
        ...
        <ul>
          {queue.map((guest, index) => (
            <li key={`${guest.id}-${index}`} className={`list-item ${index < 2 ? 'active-service' : ''}`}>
              <div className="guest-info">
                <strong>{guest.name}</strong>
                <small><Clock size={12} /> {getSlotTime(index)}</small>
              </div>
              {index < 2 && <span className="in-use-label"><CheckCircle size={14}/> In Use</span>}
              <button onClick={() => handleRemove(guest.id)} className="remove-btn"><Trash2 size={16} /></button>
            </li>
          ))}
        </ul>
      )}
