...
const ShowerBooking = () => {
  ...
  const capacityProgress = Math.min((occupied / Math.max(totalCapacity, 1)) * 100, 100);
  ...
  const waitlistToday = useMemo(
    () => (showerRecords || []).filter(
      record => record.status === 'waitlisted' && pacificDateStringFrom(record.date) === todayString
    ),
        ...
        <div className="p-4 md:p-6 space-y-6">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-900 flex gap-3">
            ...
            <div>
              ...
              <p className="leading-relaxed">Each slot supports two guests. We’ll surface the best available options first so you can confirm quickly.</p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
              <p className="text-xs uppercase text-gray-500 tracking-wide">Next availability</p>
              <p className="mt-2 text-lg font-semibold text-gray-900">
                {nextAvailableSlot ? nextAvailableSlot.label : 'No slots left'}
              </p>
              <p className="mt-2 text-xs text-gray-500">We’ll highlight remaining openings so you can book confidently.</p>
            </div>
            <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
              <p className="text-xs uppercase text-gray-500 tracking-wide">Capacity today</p>
              <div className="mt-2 flex items-baseline gap-2">
                <span className="text-2xl font-semibold text-gray-900">{occupied}</span>
              ...
              <p className="mt-3 text-xs text-gray-500 flex items-center gap-1">
                <Users size={14} />
                {available > 0 ? `${available} spots remaining` : 'All shower spots taken'}
              </p>
            </div>
            <div className="border border-gray-100 rounded-lg p-4 bg-gray-50">
              <p className="text-xs uppercase text-gray-500 tracking-wide">Waitlist today</p>
              <div className="mt-2 text-2xl font-semibold text-gray-900">{waitlistToday.length}</div>
              <p className="mt-3 text-xs text-gray-500">We’ll queue guests and notify when a slot frees up.</p>
            </div>
          </div>
          ...
          {allSlotsFull && (
            <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-900 space-y-3">
              <div className="flex items-start gap-2">
                ...
                <p>All shower slots are currently full. Add this guest to the waitlist so they’re next in line when a spot opens up.</p>
              </div>
              <button
            ...
            {guestShowerHistory.length > 0 ? (
              <ul className="space-y-3">
                {guestShowerHistory.map((record) => (
                  ...
                  >
                    <div>
                      <p className="font-medium text-gray-800">{formatDateTime(record.date)}</p>
                      <p className="text-xs text-gray-500">{record.time ? formatSlotLabel(record.time) : 'Waitlist entry'}</p>
                    </div>
