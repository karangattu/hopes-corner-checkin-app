    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        try {
          localStorage.setItem(
            `hopes-corner-${collectionName}`,
            JSON.stringify(data),
          );
        } catch (error) {
          console.warn(
            `Failed to cache ${collectionName} to localStorage:`,
            error,
          );
        }
      },
      (error) => {
        ...
        try {
          ...
          console.error(
            `Failed to load ${collectionName} from cache:`,
            fallbackError,
          );
        }
      },
  ...
  combined.forEach((item) => {
    ...
    if (
      ...
      new Date(item.lastUpdated || item.createdAt) >
        new Date(existing.lastUpdated || existing.createdAt)
    ) {
